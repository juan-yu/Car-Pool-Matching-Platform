...
public class CreateCasePassengerPanel extends JPanel {
	...
	public CreateCasePassengerPanel() throws SQLException {
		exec = new ExecSQL();
		createComp();
		display();
	}
	...
	public void createComp() {
		...
		titleLabel = new JLabel("CREATE CASE: PASSENGER");
	...
	public void createPassengerPanel() {
		passengerPanel = new JPanel();
		JLabel label = new JLabel("Number of Passenger");
		passenger = new JTextField(FIELD_WIDTH);
		passengerPanel.add(label);
		passengerPanel.add(passenger);
	}

	public void createPricePanel() {
		pricePanel = new JPanel();
		JLabel label = new JLabel("Acceptable Price Range");
	...
	public void createButPanel() {
		...
		class ButtonListener implements ActionListener {
			public void actionPerformed(ActionEvent event) {
				...
				int numOfPassenger = Integer.parseInt(passenger.getText());
				
				int lowPrice = slider.getValue();
				int highPrice = ((RangeSlider) slider).getUpperValue();

				try {
					boolean add = exec.addPassengerGroup(lowPrice, highPrice, numOfPassenger, start, destination, date);
					if(add == true) {
